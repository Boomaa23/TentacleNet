...
public class Data {
    public static List<List<Object>> SAVED_DATA_MATCHES = new ArrayList<>();
    ...
    public static void getRadioSelections(StringBuffer data, JPanel container) {
        try {
            ...
            for (Field field : displayFields) {
                if (field.getGenericType().getTypeName().equals("javax.swing.ButtonGroup")) {
        ...
        try {
            ...
            for (Field field : displayFields) {
                switch(field.getGenericType().getTypeName()) {
                    case "org.rivierarobotics.tentaclenet.OverlayField":
                        ((OverlayField) field.get(field)).reset();
                        break;
                    case "javax.swing.JTextField":
                        ((JTextField) field.get(field)).setText("");
                        break;
                    case "javax.swing.ButtonGroup":
                        ((ButtonGroup) field.get(field)).clearSelection();
                        break;
                }
            }
        } catch(IllegalArgumentException | IllegalAccessException e) {
